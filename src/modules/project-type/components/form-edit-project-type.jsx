...
export const FormEditProjectType = ({ dataDetails, setUpdate, setEdit, update }) => {
  ...
  const handeleOnSubmitUpdate = async dataProjectType => {
    ...
    dataProjectType.priority = parseInt(dataProjectType.priority);
    try {
        const respon = await axios.put(`${REACT_APP_BASE_URL}project_types/${id}`, dataProjectType);
        if (respon.status === 200) {
            setLoading(false);
            setTimeout(() => {
              setEdit(false);
              setUpdate(!update);
              Alert("Update Project Type Success", "Notification");
            }, TIMEOUT_REDIRECT);
          }
        setLoading(false);
    } catch (error) {
        setLoading(false);
        await Alert("Update Project Type Fail, try again!", "Notification");
    }
  };
  return (
              ...
              <div className="inline-block mt-2 w-1/2 pr-1">
                ...
                <div className="relative mt-2">
                  <select
                    ...
                    defaultValue={dataDetails?.priority}
