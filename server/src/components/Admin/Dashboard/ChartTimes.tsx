...
const ChartTimes = () => {
  const { t, notify, setStore } = useBaseHook();
  ...
  const fetchData = async (unit: string, startTime, endTime) => {
    setLoading(true)
    let [error, result]: any[] = await to(statisticService().withAuth().statistic({
      unit: unit,
      startTime: startTime,
      endTime: endTime
    }));
    ...
    setData(result)
    setStore("dashboard.data", result || [])
  }
